        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="text-white flex flex-col bg-slate-900 border border-slate-700/60 rounded-2xl p-6 w-full max-w-md shadow-2xl gap-5">
                ...
                <div className="flex flex-col gap-2">
                    {reportReasons.map(report => (
                        <label
                            key={report.id}
                            htmlFor={report.id}
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all duration-200 ${selectedReason === report.id
                        ...
                        >
                            ...
                            />
                            <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-all duration-200 ${selectedReason === report.id
                                ? "border-red-400"
                                : "border-slate-600"
                                }`}>
                ...
                <textarea
                    ...
                    placeholder={selectedReason === 7 ? "Írd le részletesen a problémát... (kötelező)" : "Opcionális: írd le részletesen a problémát..."}
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        disabled={!selectedReason || (selectedReason === 7 && reasonDesc == "") || loading}
                        onClick={() => sendReport()}
                        className={`flex-1 py-2.5 rounded-xl text-sm font-semibold disabled:cursor-not-allowed transition-all duration-200 ${selectedReason
