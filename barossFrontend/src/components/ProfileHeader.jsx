...
export default function ProfileHeader({ owner, user, openReportModal, setOpenReportModal }) {
    ...
    return (<>
        ...
        <div className="bg-slate-950  p-6 flex flex-col gap-6">
            ...
            {/* Fejléc kártya */}
            <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-8 flex items-center gap-8">
                ...
                <div className="flex flex-col gap-1 flex-1">
                    ...
                    <p className="text-slate-500 text-xs">{user.userClass} · Regisztráció dátuma:  {new Date(user.created_at).toLocaleDateString('hu-HU')}</p>
                ...
                {!owner &&
                    <div className="flex flex-col gap-2">
                        <button className="flex items-center gap-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 px-4 py-2.5 rounded-xl transition-all duration-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                    { label: "Aktív hirdetés", value: user.active, color: "text-blue-400" },
                    { label: "Eladott termék", value: user.sold_items, color: "text-green-400" },
                    { label: "Kedvencek", value: user.favorites, color: "text-red-400" },
                    { label: "Kapott like", value: user.liked, color: "text-yellow-400" },
                ].map((stat) => (
