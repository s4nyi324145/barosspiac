...
import { MessageCircle, Star, Tag, Flag, Bell, CheckCheck, ChevronRight, X, Trash2  } from "lucide-react"
...
export default function NotificationContainer() {
    ...
    const typeConfig = {
        new_message: { icon: MessageCircle, color: 'text-blue-400', bg: 'bg-blue-500/20', border: 'border-blue-500/30' },
        new_rating: { icon: Star, color: 'text-yellow-400', bg: 'bg-yellow-500/20', border: 'border-yellow-500/30' },
        product_sold: { icon: Tag, color: 'text-green-400', bg: 'bg-green-500/20', border: 'border-green-500/30' },
        report: { icon: Flag, color: 'text-red-400', bg: 'bg-red-500/20', border: 'border-red-500/30' },
    }
                ...
                <div className="fixed   inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-slate-900 border animate-[scale-in_0.15s_ease-out]   border-slate-700/60 rounded-2xl p-6 w-full max-w-md flex flex-col gap-5 shadow-2xl">
                        ...
                        <p className="text-slate-400 text-sm">
                            Biztosan törölni szeretnéd ezt az értesítést? A művelet véglegesen eltávolítja a listádról.
                        </p>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={() => { deleteNotification(selectedNotification.notification_id); setShowWarningModal(false) }}
                ...
                {notifications.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <div className="flex flex-col gap-1 text-center">
                            ...
                            <p className="text-slate-500 text-sm">Ha valaki üzenetet küld vagy értékel, itt fogod látni.</p>
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col gap-2">
                        {notifications.map(notification => {
                            const config = typeConfig[notification.type] || typeConfig.new_message
                            const Icon = config.icon
                            return (
                                <div
                                    key={notification.notification_id}
                                    onClick={() => markAsRead(notification)}
                                    className={`flex group items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all duration-200 hover:-translate-y-0.5 ${notification.is_read === 0
                                ...
                                >
                                    ...
                                    <div className="flex  flex-col gap-0.5 flex-1 min-w-0">
                                        <p className={`text-sm ${notification.is_read === 0 ? 'text-white font-medium' : 'text-slate-300'} truncate`}>
                                            {notification.message}
                                        </p>
                                        <p className="text-slate-500 text-xs">
                                            {new Date(notification.created_at).toLocaleString('hu-HU', { dateStyle: 'short', timeStyle: 'short' })}
                                        </p>
                                    </div>
