            ...
            {categories.map((category) => (
                <div key={category.name}>
                    ...
                    >
                        ...
                        <ChevronDown
                            className={`w-4 h-4 text-slate-500 ${filter.category === category.name ?  "text-white" : "group-hover:text-slate-300"}  transition-all duration-200 ${openCategory === category.name ? "rotate-180" : ""
                                }`}
                        />
                    </button>
                            ...
                            {category.subcategories.map((subcategory) => (
                                ...
                                <div key={subcategory.name}>
                                    ...
                                    >
                                        ...
                                        <ChevronDown
                                            className={`w-3 h-3 text-slate-600 ${filter.subcategory === subcategory.name ?  "text-white" : "group-hover:text-slate-300"} transition-all duration-200 ${openSubcategory === subcategory.name ? "rotate-180" : ""
                                                }`}
                                        />
                                    </button>
                                    ...
                                    {openSubcategory === subcategory.name && (
                                        <div className={`ml-2 border-l ${filter.subcategory === subcategory.name ?  "border-blue-700" : "border-slate-700/40"}  pl-2 flex flex-col gap-0.5 mt-2 mb-1`}>
                                            {subcategory.items.map((item) => (
                                                <button
                                                    key={item.name}
                                                    onClick={() => filter.item == item.name ? setSelectedItem(null) : setSelectedItem(item.name)}
