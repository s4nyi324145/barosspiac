...
export default function SellPage() {
    ...
    const conditions = [
        ...
        { id: "kielegito", label: "Kielégítő", desc: "Erősen használt", color: "red" },
    ]
    
    const sizes = ["XS", "S", "M", "L", "XL", "XXL"]
    const subjects = ["Töri", "Magyar", "Matek", "Földrajz", "Informatika", "Angol"]
    ...
    const isValid = form.title && form.price && form.condition &&
        form.category_id && form.sub_category_id && form.sub_sub_category_id &&
        form.collpoint && images.length > 0
                    ...
                    <div className="flex-[0.6] flex flex-col gap-6">
                        ...
                        <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-6 flex flex-col gap-5">
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-medium text-slate-300">Termék neve <span className="text-red-400">*</span></label>
                                <input
                                    type="text"
                            ...
                            {/* Leírás */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-medium text-slate-300">Leírás</label>
                                <textarea
                            ...
                            {/* Ár */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-medium text-slate-300">Ár <span className="text-red-400">*</span></label>
                        ...
                        <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-6 flex flex-col gap-5">
                            <div className="flex items-center gap-2 pb-3 border-b border-slate-800">
                                ...
                                <p className="text-white font-semibold">Kategória <span className="text-red-400">*</span></p>
                            </div>

                            <div className="flex items-center gap-2 text-sm text-slate-400">
                                <select
                                    value={form.category_id}
                                    onChange={e => setForm({ ...form, category_id: e.target.value, sub_category_id: '', sub_sub_category_id: '' })}
                                ...
                                <select
                                    ...
                                    onChange={e => setForm({ ...form, sub_category_id: e.target.value, sub_sub_category_id: '' })}
                                ...
                                >
                                    ...
                                    {categories.find(c => c.category_id == form.category_id)?.subcategories?.map(s =>
                                ...
                                <select
                                    ...
                                    onChange={e => setForm({ ...form, sub_sub_category_id: e.target.value })}
                                    className="flex-1 bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-blue-500 transition-all duration-200 appearance-none cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                    <option value="">Típus</option>
                                    {categories
                                        .find(c => c.category_id == form.category_id)
                                        ?.subcategories?.find(s => s.sub_category_id == form.sub_category_id)
                                        ?.items?.map(i =>
                        ...
                        <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-6 flex flex-col gap-5">
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-sm font-medium text-slate-300">Állapot <span className="text-red-400">*</span></label>
                            ...
                            {(form.category_id == 1 || form.category_id == 2) && (
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-medium text-slate-300">Méret</label>
                            ...
                            {form.category_id == 3 && (
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-medium text-slate-300">Tantárgy</label>
                                    <div className="flex gap-2 flex-wrap">
                                        {subjects.map(s => (
                                            <button
                                                key={s}
                                                onClick={() => setForm({ ...form, subject: form.subject === s ? '' : s })}
                                                className={`px-4 py-2 rounded-xl text-sm font-medium border transition-all duration-200 ${
                                                    form.subject === s
                            ...
                            {/* Átadás helye */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-medium text-slate-300">Átadás helye <span className="text-red-400">*</span></label>
                                <div className="flex items-center gap-3 bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-2.5 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all duration-200">
                                    ...
                                    <input
                                        ...
                                        onChange={e => setForm({ ...form, collpoint: e.target.value })}
                        ...
                        <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-6 flex flex-col gap-5 sticky top-6">
                            ...
                            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-white font-semibold">Képek <span className="text-red-400">*</span></p>
                                </div>
                            ...
                            {images.length < 5 && (
                                ...
                                >
                                    <Upload className={`w-8 h-8 ${dragOver ? 'text-blue-400' : 'text-slate-600'}`} />
                                    <div className="text-center">
                                        <p className="text-sm text-slate-400">Húzd ide a képeket</p>
                                        <p className="text-xs text-slate-600 mt-0.5">vagy kattints a feltöltéshez</p>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {images.map((img, i) => (
                                        <div key={i} className="relative  rounded-xl overflow-hidden group">
                                            <img src={img.preview} alt="Termék a fótóról" className="w-full h-full object-cover" />
                                            {i === 0 && (
