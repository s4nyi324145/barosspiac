        ...
        {conversations.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 gap-3 p-6 text-center">
            ...
            <p className="text-slate-500 text-sm">
              Még nincs egy beszélgetésed sem
            </p>
          </div>
        ) : (
          filtered.map((con) => {
            const unread = unReadMessages.find(
              (m) => m.conversations_id === con.conversations_id,
            );
            const isSelected =
              ...
              >
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    <p
                      className={`text-sm font-semibold truncate ${unread ? "text-white" : "text-slate-300"}`}
                    >
                      {con.fullname}
                    </p>
                    <p className="text-slate-600 text-xs shrink-0">
                      {new Date(
                        con.sent_at || con.created_at,
                      ).toLocaleDateString("hu-HU")}
                    </p>
                  </div>
                  <div className="flex items-center justify-between gap-1 mt-0.5">
                    <p
                      className={`text-xs truncate min-w-0 ${unread ? "text-slate-300 font-medium" : "text-slate-500"}`}
                    >
