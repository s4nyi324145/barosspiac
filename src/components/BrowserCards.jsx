...
export default function BrowserCards({ filter, setFilter }) {
    ...
    return (<>
        ...
        <div className="flex flex-col">
            {filteredProduct.length > 0 && <div className=" flex p-2 mb-4 bg-red-500/20 text-red-400 border ml-auto mt-2 border-red-500/30 rounded-full ">
                <p>{filteredProduct.length} találat</p>
            </div>}
            <div className={` ${filteredProduct.length == 0 && loading === false ? "flex flex-1" :  "grid"}  grid-cols-2 md:grid-cols-3 lg:grid-cols-4 overflow-auto scrollbar-hide  min-h-screen max-h-screen auto-rows-min  xl:grid-cols-5 gap-3 p-4 `}>
