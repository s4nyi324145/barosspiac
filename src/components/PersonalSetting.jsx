...
export default function PersonalSetting() {
  ...
  {/*Handle submit function for personal data changes*/ }
  ...
  {/*Handle profile picture upload and delete functions*/ }
  const profilePicUpload = async () => {
    ...
    try {
      ...
      console.log(error);
      showError(
        error.response?.data?.message || "Hiba történt a feltöltés során",
      );
      setUploadLoading(false);
    }
    setProfileUploadModal(false);
  ...
  {/*Handle profile picture delete function*/ }
            ...
            {profilePic && (
              <div className="flex items-center gap-4 bg-slate-800/50 border border-slate-700/50 rounded-xl p-3">
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  ...
                  <p className="text-slate-500 text-xs">
                    {(profilePic.size / 1024).toFixed(0)} KB
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="flex flex-col flex-1 p-8 text-white gap-8">
        ...
        <div className="border-b border-slate-800 pb-5">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Módosítsd a profilképed és a személyes adataidat
          </p>
        </div>
        ...
        <div className="flex items-center flex-col sm:flex-row gap-6 bg-slate-800/40 border border-slate-700/60 rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-2">
            <p className="font-semibold text-center sm:text-left text-white">
              Profilkép
            </p>
            <p className="text-slate-500 text-center sm:text-left text-xs">
              JPG vagy PNG, maximum 2MB
            </p>
            <div className="flex flex-col  sm:flex-row gap-2 mt-1">
              <button
                onClick={() => setProfileUploadModal(true)}
                className="flex items-center justify-center gap-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded-xl transition-all duration-200"
              >
                <Upload className="w-3.5 h-3.5" /> Feltöltés
              </button>
              <button
                onClick={() => deleteProfilePic()}
                className="flex items-center justify-center gap-2 text-sm font-medium text-red-400 hover:text-white border border-red-500/30 hover:bg-red-500/20 px-3 py-1.5 rounded-xl transition-all duration-200"
              >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-3 bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-2.5 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all duration-200">
              ...
              <select
                value={newClass == "" ? user.userClass : newClass}
                onChange={(e) => setNewClass(e.target.value)}
                className="w-full bg-slate-800/60 border border-slate-700/60 rounded-xl px-2 py-2 text-sm text-slate-200 outline-none focus:border-blue-500 focus:bg-slate-800 focus:ring-2 focus:ring-blue-500/20 transition-all duration-200 appearance-none cursor-pointer"
              >
          ...
          <div className="flex flex-col  gap-1.5">
            ...
            <div className="flex items-center text-center sm:text-left gap-3 flex-wrap  bg-slate-800/30 border border-slate-700/40 rounded-xl px-4 py-2.5 opacity-60 cursor-not-allowed">
              ...
              <span className="text-center flex-1 sm:max-w-fit sm:ml-auto text-xs text-slate-600 bg-red-700/50  px-2 py-0.5 rounded-full">
                Nem módosítható
              </span>
            </div>
          </div>
        ...
        <div className="flex justify-center md:justify-end">
          <button
            disabled={!newFullname || newClass === ""}
            onClick={() => handleSubmit()}
            className={`flex items-center gap-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-500 px-6 py-2.5 rounded-xl transition-all duration-200 shadow-lg shadow-blue-600/20 ${!newFullname || newClass === "" ? "opacity-50 cursor-not-allowed" : ""}`}
          >
