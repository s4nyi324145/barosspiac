...
const statusConfig = {
    pending: { label: 'Függőben', bg: 'bg-yellow-500/20', text: 'text-yellow-400', border: 'border-yellow-500/30' },
    resolved: { label: 'Lezárva', bg: 'bg-green-500/20', text: 'text-green-400', border: 'border-green-500/30' },
    rejected: { label: 'Elutasítva', bg: 'bg-slate-500/20', text: 'text-slate-400', border: 'border-slate-500/30' },
}
        ...
        <div className="p-6 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Beérkezett felhasználói jelentések kezelése</p>
                </div>
                <div className="flex gap-2">
                    {['pending', 'resolved', 'rejected'].map(s => {
                        ...
                        return (
                            <div key={s} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${config.bg} ${config.border}`}>
                                <span className={`text-xs font-medium ${config.text}`}>{config.label}</span>
                                <span className={`text-xs font-bold ${config.text}`}>{count}</span>
                            </div>
                        )
                    })}
                </div>
                    ...
                    <table className="min-w-full">
                        <thead>
                            <tr className="border-b border-slate-800">
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Típus</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Jelentő</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Jelentett</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Ok</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Státusz</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Dátum</th>
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {reports.map(report => (
                                <>
                                    ...
                                    >
                                        <td className="px-4 py-3 text-slate-500 text-sm">#{report.report_id}</td>

                                        {/* Típus */}
                                        <td className="px-4 py-3">
                                            <div className={`flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-full border text-xs font-medium ${
                                                report.product_id
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <p className="text-slate-500 text-xs">{report.reporter_email}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-sm">
                                            {report.product_id ? (
                                                <button
                                                    onClick={e => { e.stopPropagation(); navigate(`/product/${report.product_id}`) }}
                                                ...
                                                <button
                                                    onClick={e => { e.stopPropagation(); navigate(`/profile/${report.reported_id}`) }}
                                        ...
                                        <td className="px-4 py-3">
                                            {(() => {
                                                ...
                                                return (
                                                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${config.bg} ${config.text} ${config.border}`}>
                                                        {config.label}
                                                    </span>
                                                )
                                            })()}
                                        ...
                                        <td className="px-4 py-3 text-slate-500 text-sm">
                                            {new Date(report.sending_date).toLocaleString('hu-HU', { dateStyle: 'short' })}
                                        </td>
                                        ...
                                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                                            <div className="flex items-center gap-2">
                                                {report.status === 'pending' && (
                                                    <>
                                                        <button
                                                            onClick={() => handleStatus(report.report_id, 'resolved')}
                                                        ...
                                                        <button
                                                            onClick={() => handleStatus(report.report_id, 'rejected')}
                                            ...
                                            <td colSpan={8} className="px-6 py-4">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                        <p className="text-slate-400 text-xs uppercase tracking-wider">Részletes leírás</p>
            ...
            <div className="flex items-center justify-center gap-2">
                <button
                    ...
                    className={`px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                        page === 1 ? 'text-slate-600 cursor-not-allowed' : 'text-slate-300 hover:text-white border border-slate-700/60 hover:bg-slate-800'
                    }`}
                >
                    ← Előző
                </button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                    <button
                        ...
                        className={`w-9 h-9 rounded-xl text-sm font-medium transition-all duration-200 ${
                            page === p ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800'
                        }`}
                    >
                        {p}
                    </button>
                ...
                <button
                    ...
                    className={`px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                        page === totalPages ? 'text-slate-600 cursor-not-allowed' : 'text-slate-300 hover:text-white border border-slate-700/60 hover:bg-slate-800'
                    }`}
                >
                    Következő →
                </button>
