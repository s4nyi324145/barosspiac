    ...
    <>
      ...
      <div className="flex items-center justify-center gap-5 w-fit bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
        ...
        <div className="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center border text-white font-semibold text-sm shrink-0">
          {productDetail.pfp ? (
            <img
              src={productDetail.pfp}
              alt="Profilkép"
              className="w-full h-full object-cover rounded-full"
            />
          ) : (
            productDetail.fullname?.[0].toUpperCase()
          )}
        </div>
        ...
        <div className="flex flex-col gap-0.5 flex-1">
          <p
            onClick={() => navigate(`/profile/${productDetail.user_id}`)}
        ...
        <div className="flex flex-col items-end pl-5 gap-0.5">
          ...
          <p className="text-slate-500 text-xs">{ratings?.db ?? 0} értékelés</p>
        </div>
      </div>
    </>
  );
