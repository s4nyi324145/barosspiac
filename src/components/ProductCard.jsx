...
export default function ProductCard({ getLikedProducts, p }) {
  ...
  {
    /*Handle the like logic for the product */
  }
  const handleLike = (e) => {
    e.stopPropagation();
      ...
      >
        ...
        <div className="w-full h-40 bg-slate-800 relative overflow-hidden">
          ...
          <span
            className={`absolute top-2 left-2 text-xs font-medium px-2 py-0.5 rounded-full ${
              p.product_condition === "Új"
                ? "bg-green-500/20 text-green-400 border border-green-500/30"
                : p.product_condition === "Kiváló"
                  ? "bg-blue-500/20 text-blue-400 border border-blue-500/30"
                  : p.product_condition === "Jó"
        ...
        <div className="p-3 flex flex-col  gap-2">
          ...
          <div className="flex justify-between items-start gap-2">
            ...
            <span className="text-blue-400 text-sm font-bold shrink-0">
              {p.product_price?.toLocaleString("en")} Ft
            </span>
          </div>

          {/* Collpoint */}
          <p className="text-slate-500 text-xs truncate flex items-center gap-1">
            <MapPin className="text-red-600" size={16} /> {p.product_collpoint}
          </p>
        </div>
      </div>
    </>
