...
export default function ProductDesc({
  ...
  {/* Send like for that product */ }
  const sendLike = async () => {
    try {
      const result = await api.post("/likes/like", {
  ...
  {/* Remove like from that product */ }
  const removeLike = async () => {
    try {
      const result = await api.delete(
        `/likes/unlike/${productDetail.product_id}`,
      );
      showSuccess(result.data.message);
      setLiked(false);
    } catch (error) {
  ...
  {/* Handle the sold  logic for the product */ }
  ...
  {/* Handle the unsold  logic for the product */ }
      ...
      <div className="flex flex-col flex-[0.4] gap-5 md:ml-3 ml-0">
        {openMarkSoldModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div
              className="bg-slate-900 border border-slate-700/60 rounded-2xl p-6 w-full max-w-md flex flex-col gap-5 shadow-2xl"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">
                    A hirdetés eladottként lesz megjelölve
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex flex-col gap-2">
                <p className="text-red-400 text-xs font-medium">
                  Az alábbiak véglegesen törlődnek:
                </p>
                {[
                  "A hirdetés összes adata",
                  "Feltöltött képek",
                  "Beérkezett üzenetek",
                ].map((item) => (
                  <div
        ...
        <div className=" flex flex-col gap-4  lg:border-l-2  border-blue-500 text-white p-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-3xl font-bold text-blue-400">
              {productDetail.product_price.toLocaleString("en")} Ft
            </p>
          </div>
          ...
          <div className="flex gap-2 flex-wrap">
            <span
              className={`text-xs font-medium px-3 py-1 rounded-full border ${
                productDetail.product_condition === "uj"
                  ? "bg-green-500/20 text-green-400 border-green-500/30"
                  : productDetail.product_condition === "kivalo"
                    ? "bg-blue-500/20 text-blue-400 border-blue-500/30"
                    : productDetail.product_condition === "jo"
          ...
          <div className="flex items-center gap-2 text-slate-500 text-xs">
            ...
            <span>
              Feltöltve:{" "}
              {new Date(productDetail.product_upload).toLocaleDateString(
                "hu-HU",
              )}
            </span>
          </div>
          ...
          {user.user_id != productDetail.user_id ? (
            <div className="flex flex-col gap-2 mt-2">
              <button
                ...
                className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-blue-600/25 hover:-translate-y-0.5"
              >
                Érdekel →
              </button>
              {liked ? (
                <button
                  onClick={() => removeLike()}
                  className="w-full py-3.5 bg-red-800 hover:bg-red-700 border border-slate-700/60  text-slate-300 hover:text-white font-bold rounded-xl transition-all duration-200 flex items-center justify-center gap-2"
                >
                ...
                <button
                  disabled={is_sold}
                  onClick={() => (user ? sendLike() : navigate("/login"))}
                  className="w-full py-3.5 bg-slate-800 hover:bg-slate-700 border border-slate-700/60 text-slate-300 hover:text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center gap-2"
                >
