    ...
    <>
      <div>
        <div className=" border-slate-800 px-8 py-6">
          <h1 className="text-2xl font-bold text-white">Áttekintés</h1>
        </div>
        ...
        <div className="flex flex-wrap justify-center px-8  gap-6 mt-2">
          {[
            ...
            {
              value: statistics?.active_products || 0,
              label: "Aktív Hirdetés",
              icon: Tag,
              color: "green",
            },
            {
              value: statistics?.today_users || 0,
              label: "Mai Regisztrációk",
              icon: UserPlus,
              color: "yellow",
            },
            {
              value: statistics?.active_reports || 0,
              label: "Aktív Jelentések",
              icon: Flag,
              color: "red",
            },
          ].map((stat, i) => (
            <div
              key={i}
              className="flex items-center gap-4 bg-slate-900 border border-slate-700/60 px-6 py-4 rounded-2xl"
            >
              <div
                className={`w-10 h-10 rounded-xl bg-${stat.color}-500/20 border border-${stat.color}-500/30 flex items-center justify-center shrink-0`}
              >
