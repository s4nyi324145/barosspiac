...
export default function ProductImages({ product_img, is_sold }) {
  ...
  if (!product_img || product_img.length === 0)
    return (
          ...
          {product_img.length > 1 && (
            <>
              <button
                onClick={() =>
                  setSelected((prev) =>
                    prev === 0 ? product_img.length - 1 : prev - 1,
                  )
                }
              ...
              <button
                onClick={() =>
                  setSelected((prev) =>
                    prev === product_img.length - 1 ? 0 : prev + 1,
                  )
                }
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {product_img.map((img, i) => (
              <div
                ...
                className={`shrink-0 w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 transition-all duration-200 ${
                  selected === i
                    ? "border-blue-500"
