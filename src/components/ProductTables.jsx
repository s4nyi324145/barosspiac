...
export default function ProductTables({ adminProducts, setAdminProducts }) {
  ...
  {/* Handle product deletion by admin, with confirmation */}
  ...
  {/* Handle marking product as sold or active */}
  const handleMarkSold = async (product_id, is_sold) => {
    try {
      await api.put(`/product/sold/${product_id}`, { is_sold });
      setAdminProducts((prev) =>
        prev.map((p) => (p.product_id === product_id ? { ...p, is_sold } : p)),
      );
      showSuccess(
        is_sold === 1 ? "Eladottnak jelölve" : "Visszaállítva aktívra",
      );
    } catch (error) {
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              sub_sub_category_id: Number(e.target.value),
                            })
                          }
                          className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white outline-none focus:border-blue-500"
                        >
                          {categories
                            ...
                            ?.subcategories?.find(
                              (s) =>
                                s.sub_category_id === editForm.sub_category_id,
                            )
                            ?.items?.map((i) => (
                              <option
                      ...
                      {isEditing ? (
                        ...
                        <>
                          ...
                          {product.is_sold === 0 ? (
                            <button
                              onClick={() =>
                                handleMarkSold(product.product_id, 1)
                              }
                              className="p-1.5 rounded-lg text-green-400 hover:text-white hover:bg-green-500/20 transition-all duration-200"
                            ...
                            <button
                              onClick={() =>
                                handleMarkSold(product.product_id, 0)
                              }
